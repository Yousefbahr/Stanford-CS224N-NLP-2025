...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        remaining_len = len(truncated_doc) - masked_len
        prefix_len = remaining_len // 2

        prefix = truncated_doc[: prefix_len]
        suffix = truncated_doc[-(remaining_len - prefix_len):]
        masked_content = truncated_doc[prefix_len : len(truncated_doc) - len(suffix)]
