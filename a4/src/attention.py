...
def precompute_rotary_emb(dim, max_positions):
    ...
    # TODO: [part g]
    ### YOUR CODE HERE ###
    freqs = 1 / 10000 ** (-2 * (torch.arange(1, (dim // 2) + 1, 1.0)) / dim)
...
def apply_rotary_emb(x, rope_cache):
    ...
    odd_x = x[..., 1::2]
    even_x = x[..., 0::2]
    complex_x = torch.view_as_complex(torch.stack([odd_x, even_x], dim=-1))

    seq_len = x.shape[2]
    rope_cache_truncated = rope_cache[:seq_len]
    complex_rope = torch.view_as_complex(rope_cache_truncated)

    rotated_x = torch.view_as_real(complex_rope * complex_x).flatten(-2)
    ### END YOUR CODE ###
    return rotated_x
