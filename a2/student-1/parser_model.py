...
class ParserModel(nn.Module):
    ...
    def embedding_lookup(self, w):
        ...
        flatten_features = torch.flatten(w)
        all_embeddings = torch.index_select(self.embeddings, 0, flatten_features)
        flatten_embeddings = torch.flatten(all_embeddings)
        x = flatten_embeddings.view(-1 , w.shape[1] * self.embed_size)
    ...
    def forward(self, w):
        ...
        embeddings_hidden_mut = torch.matmul(embeddings, self.embed_to_hidden_weight) + self.embed_to_hidden_bias
        relu_embeddings = self.dropout(torch.nn.functional.relu(embeddings_hidden_mut))

        logits = torch.matmul(relu_embeddings, self.hidden_to_logits_weight) + self.hidden_to_logits_bias
